...
export class ReservationsController {
  ...
  @ApiResponse({ status: 200, description: 'List of requests', type: [ReservationRequestResponseDto] })
  async getMyRequests(
    @CurrentUser() user: AuthenticatedUser,
  ): Promise<ReservationRequestResponseDto[]> {
  ...
  @ApiResponse({ status: 200, description: 'Request details', type: ReservationRequestResponseDto })
  ...
  @ApiResponse({ status: 200, description: 'Request cancelled', type: ReservationRequestResponseDto })
  ...
  @ApiResponse({ status: 200, description: 'List of pending requests', type: [ReservationRequestResponseDto] })
  async getPendingRequests(
    @Param('accommodationId', ParseUUIDPipe) accommodationId: string,
    @CurrentUser() user: AuthenticatedUser,
  ): Promise<ReservationRequestResponseDto[]> {
    // TODO: Verify user owns this accommodation
    const requests = await this.reservationsService.getPendingRequestsForAccommodation(
      accommodationId,
    );
    return requests.map((r) =>
      plainToInstance(ReservationRequestResponseDto, r, {
  ...
  @ApiResponse({ status: 200, description: 'Request approved, reservation created' })
  @ApiResponse({ status: 400, description: 'Request not pending or dates conflict' })
  @ApiResponse({ status: 404, description: 'Request not found' })
  ...
  @ApiResponse({ status: 200, description: 'Request rejected', type: ReservationRequestResponseDto })
  ...
  async getMyReservations(
    ...
    const reservations = await this.reservationsService.getReservationsByGuest(user.id);
  ...
  @ApiResponse({ status: 200, description: 'Reservation details', type: ReservationResponseDto })
